class WeaponFactory {
    ...
    static createMineWeapon(entityID, parentID, entities) {

        const w = 40, h = 11;

        const c = {};
        c[RenderComponent] = new RenderComponent(entityID, {
            fill: {r:56,g:129,b:255}
        }, GameScene.WEAPON_LAYER);
        c[ShapeComponent] = new ShapeComponent(entityID, p2.Shape.BOX, {width: w, height: h}, [0,0], [0,0], 0);
        c[TransformComponent] = new TransformComponent(entityID, [0,0], 0);
        c[ParentComponent] = new ParentComponent(entityID, parentID, [0, 5], Offsets.NONE);
        c[WeaponComponent] = new WeaponComponent(entityID, 1, 1750, true, 0);
        c[ProjectileWeaponComponent] = new ProjectileWeaponComponent(entityID, 0, 0, 0.01, 0, Infinity, Infinity, Infinity, 1.5, 0, p2.Shape.BOX, {width: w, height: h}, GameScene.CHARACTER_MATERIAL, {
            onDeath: (obj) => {
                obj.scene.screenShake += 15;
            ...
            obj.components[PhysicsComponent].body.position = ParentComponent.getAbsolutePosition(entityID, entities);
            obj.components[PhysicsComponent].body.angle = 0;
            obj.components[DelayedCallbackComponent].push(
                new DelayedCallbackComponent(entityID, 1000, () => {
                    obj.components[HealthComponent].currHealth = 1
                    obj.components[RenderComponent].fill = {r:255,g:165,b:0};
                }
            )
            );
        });
